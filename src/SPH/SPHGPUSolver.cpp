...
SPHGPUSolver::SPHGPUSolver(SPHFluidParams& params, BoundaryBox& box,
    ...
    origDataToInterData.AddPartialShaderFile(sid,
                                             "ParallelSort/OriginalDataToIntermediateData.comp");
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(SPHParticle) * particles.size(), particles.data(),
                 GL_DYNAMIC_DRAW);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    glGenBuffers(1, &indexBuffer);
    std::vector<SPHParticleIndex> indexTable(params.bucketsCountX * params.bucketsCountY,
                                             SPHParticleIndex(params.particlesCount));
    for (int i = 0, local; i < params.bucketsCountY; i++) {
        for (int j = 0; j < params.bucketsCountX; j++) {
            local = 0;

            if (i - 1 >= 0 && j - 1 >= 0) {
                indexTable[i * params.bucketsCountX + j].n[local++] =
                    (i - 1) * params.bucketsCountX + (j - 1);
            }

            if (i - 1 >= 0) {
                indexTable[i * params.bucketsCountX + j].n[local++] =
                    (i - 1) * params.bucketsCountX + (j - 0);
            }
            ...
            if (j - 1 >= 0) {
                indexTable[i * params.bucketsCountX + j].n[local++] =
                    (i - 0) * params.bucketsCountX + (j - 1);
            }

            indexTable[i * params.bucketsCountX + j].n[local++] =
                (i - 0) * params.bucketsCountX + (j - 0);
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(SPHParticleIndex) * indexTable.size(),
                 indexTable.data(), GL_DYNAMIC_DRAW);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

    glGenVertexArrays(1, &vao);
    glBindVertexArray(vao);
    glBindBuffer(GL_ARRAY_BUFFER, particlesBuffer);
    glVertexAttribLPointer(0, 2, GL_DOUBLE, sizeof(SPHParticle), (void*)offsetof(SPHParticle, pos));
    glVertexAttribLPointer(1, 2, GL_DOUBLE, sizeof(SPHParticle), (void*)offsetof(SPHParticle, vel));
    glVertexAttribLPointer(2, 2, GL_DOUBLE, sizeof(SPHParticle),
                           (void*)offsetof(SPHParticle, force));
    glVertexAttribLPointer(3, 2, GL_DOUBLE, sizeof(SPHParticle),
                           (void*)offsetof(SPHParticle, prevForce));
    glVertexAttribLPointer(4, 1, GL_DOUBLE, sizeof(SPHParticle),
                           (void*)offsetof(SPHParticle, pressure));
    glVertexAttribLPointer(5, 1, GL_DOUBLE, sizeof(SPHParticle),
                           (void*)offsetof(SPHParticle, density));
    glVertexAttribLPointer(6, 1, GL_DOUBLE, sizeof(SPHParticle),
                           (void*)offsetof(SPHParticle, mass));
    ...
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, ORIGINAL_DATA_COPY_BUFFER_BINDING,
                     origDataCopyBuffer);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, origDataCopyBuffer);
    glBufferData(GL_SHADER_STORAGE_BUFFER, origDataCopy.size() * sizeof(SPHParticle),
                 origDataCopy.data(), GL_DYNAMIC_DRAW);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, prefixSum.size() * sizeof(unsigned int),
                 prefixSum.data(), GL_DYNAMIC_DRAW);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER,
                 interData.size() * sizeof(SPHParticleSortIntermediateData), interData.data(),
                 GL_DYNAMIC_DRAW);
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
...
void SPHGPUSolver::SortParticles() {
    ...
    int numWorkGroupsXByWorkGroupSize =
        numItemsInPrefixScanBuffer / PARALLEL_SORT_WORK_GROUP_SIZE_X;
    ...
    for (unsigned int bitNumber = 0; bitNumber < 32; bitNumber++) {
        ...
        unsigned int intermediateDataReadBufferOffset =
            static_cast<unsigned int>(!writeToSecondBuffer) * numItemsInPrefixScanBuffer;
        unsigned int intermediateDataWriteBufferOffset =
            static_cast<unsigned int>(writeToSecondBuffer) * numItemsInPrefixScanBuffer;

        // getting 1 bit value from intermediate data to prefix sum is 1 item per thread
        bitForPrefixScan.Enable();
        glUniform1ui(UNIFORM_LOCATION_INTERMEDIATE_BUFFER_READ_OFFSET,
                     intermediateDataReadBufferOffset);
        glUniform1ui(UNIFORM_LOCATION_INTERMEDIATE_BUFFER_WRITE_OFFSET,
                     intermediateDataWriteBufferOffset);
...
void SPHGPUSolver::UpdateParams(SPHFluidParams& params, BoundaryBox& box) {
    ...
    params.bucketsCountX =
        static_cast<uint>(std::ceil((box.xMax - box.xMin) / (params.bucketSize)));
    params.bucketsCountY =
        static_cast<uint>(std::ceil((box.yMax - box.yMin) / (params.bucketSize)));

    origDataToInterData.Enable();
    glUniform1d(origDataToInterData.GetUniformLocation("cellSize"), params.bucketSize);
    glUniform1d(origDataToInterData.GetUniformLocation("gridX"), box.xMin);
    glUniform1d(origDataToInterData.GetUniformLocation("gridY"), box.yMin);
    glUniform1ui(origDataToInterData.GetUniformLocation("cellsCount"), params.bucketsCountX);
    origDataToInterData.Disable();

    accumulateForces.Enable();
    glUniform1d(accumulateForces.GetUniformLocation("h"), params.effectiveRadius);
    glUniform1d(accumulateForces.GetUniformLocation("mu"), params.viscocity);
    glUniform1d(accumulateForces.GetUniformLocation("s_tens_coeff"), params.surfaceTension);
    glUniform1d(accumulateForces.GetUniformLocation("s_tens_tresh"), params.tensionTreshold);
    glUniform1d(accumulateForces.GetUniformLocation("g"), -9.81f);
    glUniform1ui(accumulateForces.GetUniformLocation("index_max_neighbors"),
                 params.avgKernelParticles);
    accumulateForces.Disable();

    densityAndPressure.Enable();
    glUniform1d(densityAndPressure.GetUniformLocation("h"), params.effectiveRadius);
    glUniform1d(densityAndPressure.GetUniformLocation("k"), params.stiffness);
    glUniform1ui(densityAndPressure.GetUniformLocation("rd"), params.restDensity);
    glUniform1ui(densityAndPressure.GetUniformLocation("index_max_neighbors"),
                 params.avgKernelParticles);
    ...
    glUniform1ui(indexClear.GetUniformLocation("indexTableSize"),
                 params.bucketsCountX * params.bucketsCountY);
    indexClear.Disable();

    leapfrogIntegration.Enable();
    glUniform1d(leapfrogIntegration.GetUniformLocation("dt"), params.dt);
    leapfrogIntegration.Disable();

    boundaries.Enable();
    glUniform1d(boundaries.GetUniformLocation("damping_coeff"), 0.6f);
    glUniform1d(boundaries.GetUniformLocation("boxMinX"), box.xMin);
    glUniform1d(boundaries.GetUniformLocation("boxMaxX"), box.xMax);
    glUniform1d(boundaries.GetUniformLocation("boxMinY"), box.yMin);
    glUniform1d(boundaries.GetUniformLocation("boxMaxY"), box.yMax);
    boundaries.Disable();
