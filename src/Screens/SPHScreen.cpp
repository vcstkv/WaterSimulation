...
void SPHScreen::Render(const Graphics& graphics) {
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    fluid->Render(graphics, projection, view);
    if (!isGUI) {
        return;
    }
    fnt->DrawText(L"density: " + std::to_wstring(restDensitySlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 50 + 24 + 10, projection);
    restDensitySlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"viscosity: " + std::to_wstring(viscositySlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 2 * 50 + 24 + 10, projection);
    viscositySlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"stiffness: " + std::to_wstring(stiffnessSlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 3 * 50 + 24 + 10, projection);
    stiffnessSlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"sTension: " + std::to_wstring(surfaceTensionSlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 4 * 50 + 24 + 10, projection);
    surfaceTensionSlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"mass: " + std::to_wstring(massSlider->GetValue()), 24, glm::vec4(0, 0, 0, 1), 0,
                  height - 5 * 50 + 24 + 10, projection);
    massSlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"maxVel: " + std::to_wstring(maxVelSlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 6 * 50 + 24 + 10, projection);
    maxVelSlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"maxAcc: " + std::to_wstring(maxAccSlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 7 * 50 + 24 + 10, projection);
    maxAccSlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"avgKerParticles: " + std::to_wstring(avgKernelParticlesSlider->GetValue()), 24,
                  glm::vec4(0, 0, 0, 1), 0, height - 8 * 50 + 24 + 10, projection);
    avgKernelParticlesSlider->Draw(graphics, projection, guiview);
    fnt->DrawText(L"FPS: " + std::to_wstring(fps), 24, glm::vec4(0, 0, 0, 1), width - 100,
                  height - 30, projection);
...
void SPHScreen::OnMouseCursorEvent(float x, float y) {
...
void SPHScreen::OnMouseScrollEvent(float offsetX, float offsetY) {}
...
void SPHScreen::Initialize() {
    ...
    p.viscocity = 50.;
    p.dt = 0.001;
    p.stiffness = 20.;  // 0.3;//3.5;
    p.surfaceTension = 0.8;
    p.avgKernelParticles = 40;
    p.maxAcc = 100.;
    p.maxVel = 0.8;
    p.tensionTreshold = std::sqrt(p.restDensity / p.avgKernelParticles);
    const auto one_over_pi = glm::one_over_pi<double>();
    p.particleMass = 0.02;
    p.particlesCount = 8192;  /// 30000;// 65536;//32768;/*8192;*/ //10000;
    p.particleRadius =
        2. *
        std::sqrt(p.particleMass * one_over_pi /
                  p.restDensity);  //*/std::cbrt(0.75 * p.particleMass * M_1_PI / p.restDensity);
    p.effectiveRadius =
        3. * std::sqrt(p.avgKernelParticles *
                       /*p.particleRadius * p.particleRadius*/ p.particleMass * one_over_pi /
                       p.restDensity);  // p.particleRadius * 3.5;*/std::cbrt(0.75 * p.particleMass
                                        // * M_1_PI * p.avgKernelParticles / p.restDensity);
    // p.restDensity = 100000.f;

    BoundaryBox box = {p.particleRadius,
    ...
    surfaceTensionSlider =
        new Slider(200.f, height - 4.f * 50.f, 400.f, 20.f, glm::vec4(1, 0, 0, 1));
    ...
    avgKernelParticlesSlider =
        new Slider(200.f, height - 8.f * 50.f, 400.f, 20.f, glm::vec4(1, 0, 0, 1));
    ...
    restDensitySlider->SetCallback(
        [](double value, void* data) {
            ...
            ParamSliderData* sd = static_cast<ParamSliderData*>(data);
            sd->p->restDensity = value;
            // sd->p->particleRadius = sqrtf(sd->p->particleMass * M_1_PI / sd->p->restDensity);
            // sd->p->effectiveRadius = sqrtf(sd->p->avgKernelParticles * sd->p->particleRadius *
            // sd->p->particleRadius);
    ...
    massSlider->SetCallback(
        [](double value, void* data) {
            ...
            const auto one_over_pi = glm::one_over_pi<double>();
            sd->p->particleRadius =
                std::sqrt(sd->p->particleMass * one_over_pi / sd->p->restDensity);
            sd->p->effectiveRadius = std::sqrt(sd->p->avgKernelParticles * sd->p->particleRadius *
                                               sd->p->particleRadius);
            sd->f->AdjustParams(*sd->p);
        },
        &sd);
    ...
    maxAccSlider->SetCallback(
        [](double value, void* data) {
            ...
            auto* sd = static_cast<ParamSliderData*>(data);
            sd->p->maxAcc = value;
            sd->f->AdjustParams(*sd->p);
        },
        &sd);
    avgKernelParticlesSlider->SetCallback(
        [](double value, void* data) {
            ...
            sd->p->tensionTreshold = std::sqrt(sd->p->restDensity / sd->p->avgKernelParticles);
            sd->p->effectiveRadius = std::sqrt(sd->p->avgKernelParticles * sd->p->particleRadius *
                                               sd->p->particleRadius);
            sd->f->AdjustParams(*sd->p);
        },
